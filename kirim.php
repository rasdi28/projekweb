...
if(empty($attachment)){ // Jika tanpa attachment
    $kirim = $mail->kirim();
    if($kirim){ // Jika Email berhasil dikirim
        ...
        // echo '<h1>ERROR<br /><small>Error while kiriming email: '.$mail->getError().'</small></h1>'; // Aktifkan untuk mengetahui error message
    }
    ...
    if($size <= 25000000){ // Jika ukuran file <= 25 MB (25.000.000 bytes)
        ...
        if($kirim){ // Jika Email berhasil dikirim
            ...
            echo "<h1>Email gagal dikirim</h1><br /><a href='index.php'>Kembali ke Form</a>";
            // echo '<h1>ERROR<br /><small>Error while kirimending email: '.$mail->getError().'</small></h1>'; // Aktifkan untuk mengetahui error message
        }
